...
export default function AdminPage() {
  ...
  if (!token) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-slate-950 p-6">
        ...
        <div className="w-full max-w-md bg-slate-900/40 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-8 shadow-2xl relative">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-xs text-slate-400 mt-1">Access secure system analytics & keys</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-5">
            {error && (
              <div className="text-xs font-semibold text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-lg p-3">
                {error}
              </div>
            )}

            <div>
              <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-2">Username</label>
              <input
                type="text"
                value={username}
                onChange={e => setUsername(e.target.value)}
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-violet-500 transition-colors"
                placeholder="admin"
                required
              />
            </div>
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-2">Password</label>
              <input
                type="password"
                value={password}
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-xl py-6 flex items-center justify-center shadow-lg shadow-violet-600/20 transition-all"
            >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10 relative">
      ...
      {/* Header */}
      <header className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800/80 mb-6 relative">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
              OmniKey AI <span className="text-[10px] uppercase tracking-wider font-extrabold px-2 py-0.5 rounded bg-violet-500/20 text-violet-400 border border-violet-500/30">Console</span>
            </h1>
            <p className="text-xs text-slate-400 mt-0.5">Dual-database cloud admin system management panel</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button
            ...
            className="text-xs font-semibold text-slate-300 border-slate-800 hover:bg-slate-900 bg-slate-900/40 rounded-xl h-9 px-4"
          >
            {statsLoading ? 'Refreshing...' : 'Refresh Stats'}
          </Button>
          <Button
            onClick={handleLogout}
            className="text-xs font-semibold bg-rose-600 hover:bg-rose-500 text-white rounded-xl h-9 px-4 shadow-lg shadow-rose-600/10 transition-all"
          >
            Log Out
          </Button>
        </div>
      ...
      {/* Navigation tabs row */}
      <section className="max-w-7xl mx-auto flex items-center border-b border-slate-800 mb-8">
      ...
      <main className="max-w-7xl mx-auto space-y-8 relative">
        {error && (
          <div className="text-xs font-semibold text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-xl p-4">
            {error}
          </div>
        )}
        {successMsg && (
          <div className="text-xs font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
            {successMsg}
          </div>
        )}
          ...
          <>
            ...
            <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-slate-900/30 border border-slate-800/80 rounded-xl p-5 shadow-sm">
                ...
                <div className="text-2xl font-bold text-white mt-1.5">{stats?.system?.totalUsers ?? '—'}</div>
                <div className="text-[10px] text-slate-500 mt-1">Registered developer accounts</div>
              </div>

              <div className="bg-slate-900/30 border border-slate-800/80 rounded-xl p-5 shadow-sm">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">API Keys</span>
                <div className="text-2xl font-bold text-white mt-1.5">{stats?.system?.totalKeys ?? '—'}</div>
                <div className="text-[10px] text-emerald-400 flex items-center gap-1 mt-1">
                  <span>{stats?.system?.activeKeys ?? 0} healthy / active</span>
                </div>
              </div>

              <div className="bg-slate-900/30 border border-slate-800/80 rounded-xl p-5 shadow-sm relative overflow-hidden">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Overall Savings</span>
                <div className="text-2xl font-bold text-emerald-400 mt-1.5">${stats?.system?.overallCostSaved !== undefined ? stats.system.overallCostSaved.toFixed(2) : '—'}</div>
                <div className="text-[10px] text-slate-500 mt-1">Free-tier value mapped</div>
              </div>

              <div className="bg-slate-900/30 border border-slate-800/80 rounded-xl p-5 shadow-sm">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Avg saved / Request</span>
                ...
                <div className="text-[10px] text-slate-500 mt-1">Average value per routing</div>
              </div>
            </section>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-slate-900/30 border border-slate-800/80 rounded-xl p-5 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-xl font-bold text-white mt-1 block">{stats?.system?.totalRequests ?? '—'}</span>
                </div>
                <div className="h-10 w-10 rounded-lg bg-slate-800/60 flex items-center justify-center text-slate-400">
              ...
              <div className="bg-slate-900/30 border border-slate-800/80 rounded-xl p-5 flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Overall Success Rate</span>
                  <span className="text-xl font-bold text-emerald-400 mt-1 block">
                    {stats?.system?.successRate !== undefined ? `${stats.system.successRate.toFixed(1)}%` : '—'}
                  </span>
                </div>
                <div className="h-10 w-10 rounded-lg bg-emerald-500/5 flex items-center justify-center text-emerald-400 border border-emerald-500/10">
              ...
              <div className="bg-slate-900/20 border border-slate-800/80 rounded-xl p-6">
                ...
                {stats?.latencyDistribution ? (
                  <div className="space-y-4">
                    {[
                      { label: 'Instant (<200ms)', key: 'fast', color: 'from-emerald-600 to-teal-500' },
                      { label: 'Standard (200ms - 1s)', key: 'normal', color: 'from-violet-600 to-indigo-500' },
                      { label: 'Delayed (1s - 3s)', key: 'slow', color: 'from-amber-600 to-orange-500' },
                      { label: 'Severe (>3s)', key: 'verySlow', color: 'from-rose-600 to-red-500' }
                    ].map(bucket => {
                      ...
                      return (
                        <div key={bucket.key} className="space-y-1.5">
                          <div className="flex justify-between text-xs font-semibold">
                            ...
                            <span className="text-slate-400">{count} queries</span>
                          </div>
                ...
                {stats?.errorBreakdown && stats.errorBreakdown.length > 0 ? (
                  <div className="space-y-3">
                    {stats.errorBreakdown.map((err, index) => (
                      <div key={index} className="bg-slate-950/40 border border-slate-800/40 rounded-xl p-3.5 flex justify-between items-center gap-3">
                        <span className="text-xs font-mono text-rose-300 truncate max-w-[80%]">{err.error}</span>
                  ...
                  <div className="text-xs text-slate-500 text-center py-10">Zero system routing errors detected. Healthy state!</div>
                )}
              </div>
            </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <section className="bg-slate-900/20 border border-slate-800/80 rounded-xl p-6">
                <h2 className="text-sm font-bold text-white mb-5 uppercase tracking-wider flex items-center gap-2">
                ...
                {stats?.platformBreakdown && stats.platformBreakdown.length > 0 ? (
                  <div className="space-y-4">
                    {stats.platformBreakdown.map(p => (
                      <div key={p.platform} className="bg-slate-900/50 border border-slate-800/50 rounded-xl p-4 space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-bold text-slate-200 capitalize">{p.platform}</span>
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div className="bg-slate-950/40 p-2 rounded-lg">
                            ...
                            <div className="text-xs font-bold text-white mt-1">{p.totalRequests}</div>
                          </div>
                          <div className="bg-slate-950/40 p-2 rounded-lg">
                            <div className="text-[10px] text-slate-500 font-medium uppercase">Success</div>
                            <div className="text-xs font-bold text-white mt-1">{p.successRate.toFixed(1)}%</div>
                          </div>
                          <div className="bg-slate-950/40 p-2 rounded-lg">
                            <div className="text-[10px] text-slate-500 font-medium uppercase">Avg Latency</div>
                            <div className="text-xs font-bold text-white mt-1">{p.avgLatencyMs}ms</div>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    {stats.modelBreakdown.map((m) => {
                      ...
                      return (
                        <div key={m.modelId} className="space-y-1.5">
                          <div className="flex justify-between text-xs font-semibold">
                            <span className="text-slate-300 font-mono text-[11px]">{m.modelId}</span>
                            <span className="text-slate-400 text-[10px]">{m.totalRequests} hits</span>
                          </div>
                          <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden">
                            <div 
            ...
            <section className="bg-slate-900/20 border border-slate-800/80 rounded-xl p-6">
              ...
              {stats?.timeSeries && stats.timeSeries.length > 0 ? (
                <div className="flex items-end gap-2.5 h-36 pt-4 border-b border-slate-800">
                  {stats.timeSeries.map((t) => {
                    ...
                    return (
                      <div key={t.date} className="flex-1 flex flex-col items-center group relative cursor-pointer">
                        <div className="absolute -top-8 bg-slate-900 border border-slate-800 text-[10px] px-2 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody>
                    {stats?.users && stats.users.length > 0 ? (
                      stats.users.map(u => (
                        <tr key={u.userId} className="border-b border-slate-800/40 hover:bg-slate-900/10 transition-colors">
                          <td className="py-4 pl-4 font-semibold text-slate-200">
                            <div>{u.email}</div>
                            <div className="text-[10px] text-slate-500 font-mono mt-0.5">UID: {u.userId}</div>
                          </td>
                          <td className="py-4 text-center text-slate-300 font-semibold">{u.keysCount} keys</td>
                          <td className="py-4 text-center text-slate-300 font-semibold">{u.requestsCount} reqs</td>
                          <td className="py-4 text-center text-slate-400 font-mono">{(u.tokensConsumed / 1000).toFixed(1)}k</td>
                          <td className="py-4 text-right pr-4 font-bold text-emerald-400">${u.costSaved.toFixed(2)}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan={5} className="py-10 text-slate-500 text-center">No active user accounts.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          <section className="bg-slate-900/20 border border-slate-800/80 rounded-xl p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Enable or disable specific language models globally from proxy loadbalancing.</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {stats?.modelsCatalog && stats.modelsCatalog.length > 0 ? (
                stats.modelsCatalog.map(model => {
                  ...
                  return (
                    <div key={key} className="bg-slate-900/40 border border-slate-800/80 rounded-xl p-4 flex items-center justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-white">{model.displayName}</span>
                          <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded font-extrabold bg-slate-850 text-slate-400 border border-slate-800">
                            {model.platform}
                          </span>
                        </div>
                        <div className="text-[10px] text-slate-500 font-mono mt-1 select-all">{model.modelId}</div>
                      </div>
                      ...
                      <Button
                        ...
                        className={`text-xs font-semibold rounded-xl px-4 py-2 border transition-all h-8 ${
                          model.enabled
                            ...
                            : 'bg-rose-500/10 border-rose-500/20 hover:border-rose-500/40 text-rose-400'
                        }`}
                      >
                ...
                <div className="col-span-2 text-xs text-slate-500 text-center py-10">No models found in database catalog catalog.</div>
              )}
            </div>
          </section>
        )}
          ...
          <section className="bg-slate-900/20 border border-slate-800/80 rounded-xl p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Audit the 15 most recent routing request events processed through the proxy.</p>
              </div>
              <Button
                disabled={flushingLogs}
                onClick={handleFlushLogs}
                className="bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold rounded-xl h-9 px-4 shadow-lg shadow-rose-600/10 transition-all self-start sm:self-center"
              >
                ...
                <tbody>
                  {stats?.recentLogs && stats.recentLogs.length > 0 ? (
                    stats.recentLogs.map((log, index) => (
                      <tr key={index} className="border-b border-slate-800/40 hover:bg-slate-900/10 transition-colors">
                        <td className="py-4 pl-4 font-mono text-[10px] text-slate-400 whitespace-nowrap">
                          {new Date(log.createdAt).toLocaleString()}
                        </td>
                        <td className="py-4 font-mono text-[10px] text-slate-400 select-all">
                          {log.userId}
                        </td>
                        <td className="py-4">
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs font-semibold text-slate-200">{log.modelId}</span>
                            <span className="text-[9px] uppercase tracking-wider font-extrabold px-1 rounded bg-slate-950 text-slate-500">
                              {log.platform}
                            </span>
                          </div>
                          {log.error && (
                            <div className="text-[10px] text-rose-400 mt-1 max-w-xs truncate font-mono">{log.error}</div>
                          )}
                        </td>
                        ...
                        <td className="py-4 text-center text-slate-300 font-semibold">{log.latencyMs}ms</td>
        ...
        {/* -------------------- 4. SECURITY TAB -------------------- */}
        {activeTab === 'security' && (
          <section className="max-w-md bg-slate-900/20 border border-slate-800/80 rounded-xl p-6">
            ...
            <form onSubmit={handleUpdateCredentials} className="space-y-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-2">New Username</label>
                <input
                  type="text"
                  value={newUsername}
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-2">New Password</label>
                <input
                  type="password"
                  value={newPassword}
              ...
              <Button
                ...
                className="w-full bg-violet-600 hover:bg-violet-500 text-white font-semibold rounded-xl py-5 flex items-center justify-center shadow-lg shadow-violet-600/20 transition-all text-xs"
              >
