...
proxyRouter.post('/images/generations', async (req: Request, res: Response) => {
  ...
  await dbModeStorage.run(auth.isLocal ? 'local' : 'cloud', async () => {
    try {
      ...
      if (requestedModel && typeof requestedModel === 'string' && requestedModel !== 'auto') {
      ...
      if (!preferredModelId) {
        res.status(400).json({
          error: { message: `Image model '${targetModelId || 'auto'}' is not found or is disabled.`, type: 'invalid_request_error' },
        });
        return;
      }
